...
# Load 'regional.xlsx'
regional_wb = openpyxl.load_workbook('I:\Openpyxl_tutorial\Parts\Dec_3_regional(NEW MINE) - Copy.xlsx')
regional_sheet = regional_wb.active


read_file = pd.read_csv(r'I:\Openpyxl_tutorial\Parts\Dec 3 Site to CC.csv')
read_file.to_excel(r'Dec 3 Site to CC(converted).xlsx', index=None, header=True)

# Load 'Site to CC.xlsx'
site_to_cc_wb = openpyxl.load_workbook('I:\Openpyxl_tutorial\Parts\Dec 3 Site to CC(converted).xlsx')
site_to_cc_sheet = site_to_cc_wb.active

print('checking..')
on_air_site = openpyxl.load_workbook('I:\Openpyxl_tutorial\Parts\ON AIR SITES DETAILS _230930.xlsx')
print('checking............')
...
for rows in regional_sheet.iter_rows():
    if rows[13].value.strip().lower() == '#N/A'.lower() or rows[19].value.strip().lower() == '#N/A'.lower():
        rows[20].value = '#N/A'
...
for rows in regional_sheet.iter_rows():
    ...
    if district is not None and calculated_district is not None and are_strings_similar(district, calculated_district, max_allowed_diff=2):
...
regional_wb.save('regional_file_with_calculated_district_tf_cols.xlsx')
...
regional_wb.save('regional_updated7.xlsx')
